<div class="space-y-3" @if(!is_null($question->parent_id)) data-conditional-parent="{{$question->parent->id}}" data-conditional-value="{{implode(',', $question->parent_values)}}" @endif>
    ...
    <textarea id="{{$question->id}}"
              ...
              class="w-full p-3 border
                     ...
                     rounded-xl bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 transition">{{old($question->id, $answers[0] ?? null)}}</textarea>
    @error($question->id)
